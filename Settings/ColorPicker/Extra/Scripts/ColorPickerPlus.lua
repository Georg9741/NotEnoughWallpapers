...
-- set alpha to the given string, or set the given alpha property to the given value
function SetAlpha(...)

    if arg.n == 1 then
        colors.cur_alpha = arg[1]
    else
        colors['cur_' .. arg[1]] = Clamp((SKIN:ParseFormula(arg[2]) * 255),0,255)
        colors.cur_alpha = string.format('%s')
    end
...
-- change an alpha property by the given delta
function ChangeAlpha(key, delta)

    SetAlpha(key, Clamp(colors['cur_' .. key] + delta, 0, 255), true)
